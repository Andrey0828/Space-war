...
"Класс, контролирующий игру: спавн пришельцев и монет, соприкосновения, отправка на экран смерти"
...
def start_screen(width, height):
    ...
    def draw_coin_on_start_screen():
        ...
        coin_draw.set_pos(739, 10)

        draw_text(screen, ': ', 45, (781, 13))
        draw_text(screen, str(old_coin), 45, (809, 15))
...
"Класс, для обычных пришельцев, двигающихся по y"
...
"Класс, для обычных пришельцев, двигающихся по x и y"
...
"Класс, для пришельцев, которые останавливаются и стреляют в игрока"
...
"Класс, для пришельцев, которые двигаются по x за игроком и стреляют в него"
...
class Player(SWSprite):
    ...
    "Стрельба игрока, в зависимости от типа корабля"
